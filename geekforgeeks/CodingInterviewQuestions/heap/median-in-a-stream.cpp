...
{
    // in the max heap we add the values that are lower then the current max values
    // so that we get max of the lowest value in the stream
    // similarly in the min heap we add all the greater value so that we get the min value of the the max values
