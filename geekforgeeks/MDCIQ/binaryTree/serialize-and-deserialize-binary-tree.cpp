...
public:
    vector<int> serialize(Node *root)
    {
        ...
        for (int i = 0; i < leftSerialized.size(); i++)
        {
            ans.push_back(leftSerialized[i]);
        }

        for (int i = 0; i < rightSerialized.size(); i++)
        {
            ans.push_back(rightSerialized[i]);
        }

        return ans;
    }

    Node *deSerializeHelper(queue<int> q)
    {
        if (q.empty() == true)
        {
            return NULL;
